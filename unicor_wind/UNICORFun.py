...
# --------------------------------------------------------------------------
def RasterToWindSpeedDirection(resgrid,elevgrid,barrgrid,windmaxres):
	'''
	...
	# Get min and max resistance for scaling
	max_res = float(windmaxres)
	min_res = 1.
	
	# Convert speed to resistance
	speed_res = ((min_res - max_res) / (max_speed - min_speed)) * (nWeightPairs[:,:,:,3] - max_speed) + 1
			...
			if edge_type == 'all_paths':
				...
				for j in range(len(vOrder)):
					...
					if resans: # Use resistance
						#if dirtype == 'Wind':
						#tupCalcOut = calcOutputs_wind(indexDict,srclist[i],vOrder[j],path[j],nWeightPairs,xvalues,yvalues,pathadd,cdmatrix,paths,pathlens[j],pathcond[j],cdmatrix_accCost)
						#else:
