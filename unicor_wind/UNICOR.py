...
#------------------------------------------------------------ 
	...
	windmaxres = r.kwdGetValue('Wind_Resistance_Max')
	...
	# Call the function to get directional graph with resistance values
		...
		if resans: #Use resistance surface
			if dirtype == 'Wind': # Sum up windspeeds
				nWeightPairs, xrasloc,yrasloc, header_dict = RasterToWindSpeedDirection(resgrid,elevgrid,barrgrid,windmaxres)
			else: # Traditional least cost direction
