...
# ------------------------------------------------------------------------------------------------------
appRele = "2014-11-07T12:07:00MDT"
appName = "UNICOR"
...
#------------------------------------------------------------ 
	...
	barrfile = r.kwdGetValue('Barrier_Filename')
	elevfile = r.kwdGetValue('Direction_Filename')
	#permans = r.kwdGetValue('Permeability')
	...
	# Call the function to get directional graph with resistance values
	else:		
		# Prepare resistance grid files
		resgrid = FileIO.PrepTextFile(gridfile)
		elevgrid = FileIO.PrepTextFile(elevfile)
		barrgrid = FileIO.PrepTextFile(barrfile)
	...
	if os.name == 'nt' or num_of_pro == 1: 
		...
		FileIO.logMsg(logfHndl,stringout)
		if not directionans:
			tupSpaths = serial_paths(logfHndl,nopoints,\
			...
			tupSpaths_upper = serial_paths(logfHndl,nopoints,stringpts_rev,visited_upper,EDthresholdans,nWeightPairs,xvalues,yvalues,pathadd_upper,cd_matrix_upper,paths_upper,edge_type,edge_dist,transform_func,const_kernal_vol,vol_constant,nbhd_dist,resans)
	...
	# If not windows, then run parallel paths
		...
		if directionans:
			...
			stringpts,visited_upper,EDthresholdans,num_of_pro \
	...
	if not directionans:
		...
		if resans: # Resistance
			cd_matrix_file_name_rev = barrfile + '_' + input_file_name + '_' + input_point_name + 'asymmetric'+cd_matrix_ext 
	...
	if outputPathadd:
		if not directionans:
			...
			#output_pathadd(logfHndl,pathadd_rev,pathadd_file_name_rev,header_dict,keys,data_dir)
